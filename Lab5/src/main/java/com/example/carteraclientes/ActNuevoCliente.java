            ...
            case R.id.action_ok:
                if(bCamposCorrectos()){
                    try {
                        ...
                        values.put(Datos.COL_1, edtNombre.getText().toString().trim());
                        values.put(Datos.COL_2, edtDireccion.getText().toString().trim());
                        values.put(Datos.COL_3, edtEmail.getText().toString().trim());
                        values.put(Datos.COL_4, edtTelefono.getText().toString().trim());

                        conexion.insert(Datos.TABLE_NAME, null, values);
                        conexion.close();
                        finish();
                    } catch (Exception ex){
